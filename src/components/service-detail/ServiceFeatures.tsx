      ...
      >
        {service.keyHighlights.map((hl) => (
          ...
          >
            ...
            <p style={{ fontSize: '0.86rem', color: 'var(--color-text-secondary)', lineHeight: 1.55, margin: 0 }}>
              {hl.description}
            </p>
          </div>
        ))}
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: 'var(--space-4)' }}>
            ...
            <h3 style={{ fontSize: '1.15rem', fontFamily: 'var(--font-serif, "Domine", serif)', fontWeight: 600, color: 'var(--color-text-primary)', margin: 0 }}>
              What Is Included
            </h3>
          </div>
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            {service.inclusions.map((item, idx) => (
              <li key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.55rem', fontSize: '0.88rem', color: 'var(--color-text-primary)' }}>
        ...
        >
          ...
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            {service.exclusions.map((item, idx) => (
              <li key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.55rem', fontSize: '0.88rem', color: 'var(--color-text-secondary)' }}>
                <span style={{ color: 'var(--color-text-muted)', marginTop: '2px' }}>✕</span>
                <span>{item}</span>
              </li>
            ))}
          </ul>
